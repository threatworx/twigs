...
NMAP_HTTP_PORTS = "80,443,8080,8443" 
NSE_HTTP_PATH = "/"+os.path.dirname(os.path.realpath(__file__)) + '/nse/http/'
NSE_OTHER_PATH =  "/"+os.path.dirname(os.path.realpath(__file__)) + '/other/' 
NSE_DB_PATH  = "/"+os.path.dirname(os.path.realpath(__file__)) + '/database/'
NMAP_DB_PORTS = "9200,9300,27017,27018,27019,3306,7000,7001,9042,7199,523" #based off mongodb, mysql, cassandra, elasticsearch, db2-das-info
...
def create_nmap_cmd (args):
    if args.servicetype is None: 
        cmd = NMAP + ' -oX - -p'+NMAP_PORTS+' -A --script '+NSE_HTTP_PATH+NSE_OTHER_PATH+NSE_DB_PATH+',http-generator,amqp-info,mongodb-info,http-wordpress-enum,mysql-info,smb-os-discovery -T' + args.timing
        return cmd
    ports = ""
    scripts = ""
    os = ""
    ...
    if "database" in args.servicetype:
        ports += NMAP_DB_PORTS
        scripts += NSE_DB_PATH+',mongodb-info,mysql-info,cassandra-info,db2-das-info' 
    if "http" in args.servicetype:
        ports += NMAP_HTTP_PORTS
        scripts += NSE_HTTP_PATH+',http-generator,http-wordpress-enum'
    if "os" in args.servicetype: 
        os = " -O"
        scripts += ",smb-os-discovery"
        if ports == "":
            ports += "1-100" 
    cmd = NMAP + ' -oX - -T ' + args.timing
    if ports != "":
        cmd += ' -p' + ports
    if scripts != "" :
        scripts = scripts[1:]
...
#try few services shodan pick server and do detection - test out all the os detection 
#single ip address  -test all scripts we have 
