...
def get_snmp_oid_value(args, snmpwalk, oid):
    ...
    try:
        ...
        logging.error("Exception running snmpwalk command")
        return None
    try:
        if ':' in out:
...
def _run_nmap(args, host):
    ...
    try:
        ...
        return out.decode(args.encoding)
    except subprocess.CalledProcessError:
        logging.error("Error running nmap command")
        return None


def _parse_host_ports(h):
    ports_in_use_dict = {}
    for port in h.getElementsByTagName("port"):
        ...
        if service is None or len(service) == 0:
            continue
        service = service[0]
        service_product = service.getAttribute('product') if service is not None else None
        service_name = service.getAttribute('name') if service is not None else None
        ports_in_use_dict[port_no] = {"port": str(port_no), "protocol": protocol, "service": service_name, "product": service_product}
    return ports_in_use_dict


def _parse_host_cpes(h):
    products = []
    for c in h.getElementsByTagName("cpe"):
        ...
        if len(carr) >= 5:
            prodstr += carr[4]
        prodstr = prodstr.strip().replace('_', ' ')
        if prodstr not in products:
            products.append(prodstr)
    return products


def _process_snmp_sysdescr(args, cmd, prod, products, ostype):
    ...
    return products, ostype


def _process_services(args, h, addr, products, ostype):
    ...
    for s in services:
        if s.getAttribute('name') == 'snmp' and ('snmp' in args.services or 'ot' in args.services):
            ...
            if sysdescr:
                ...
                products, ostype = _process_snmp_sysdescr(args, cmd, sysdescr, products, ostype)
...
def _process_scripts(h, hostname, products, ostype):
    scripts = h.getElementsByTagName("script")
    for s in scripts:
        script_id = s.getAttribute('id')
        if script_id == 'http-wordpress-enum':
            ...
            if wpout != None and wpout != '':
                ...
                for prod in prods:
                    ...
                    if prod not in products:
                        products.append(prod)
        elif script_id == 'http-php-version':
            wpout = s.getAttribute('output')
            if wpout != None and wpout != '':
                if ':' not in wpout:
                    continue
                ...
                if prodstr not in products:
                    products.append(prodstr)
        elif script_id == 'mirth-connect-version':
            wpout = s.getAttribute('output')
            if wpout != None and wpout != '':
                prodstr = 'mirth connect ' + wpout.split(':')[1].strip()
                if prodstr not in products:
                    products.append(prodstr)
        elif script_id == 'connectwise-screenconnect-version':
            wpout = s.getAttribute('output')
            if wpout != None and wpout != '':
            ...
            for e in elems:
                ...
                if key and key == 'Version':
                    ...
                    if prodstr not in products:
                        products.append(prodstr)
        elif script_id == 'erldp-info':
            wpout = s.getAttribute('output')
            if wpout != None:
                prodstr = 'erldp ' + wpout.split('version:')[1].split('node')[0].strip()
                if prodstr not in products:
                    products.append(prodstr)
        elif script_id == 'confluence-version':
            wpout = s.getAttribute('output')
            if wpout != None:
        ...
        elif script_id == 'goanywhere_version':
            wpout = s.getAttribute('output')
            if wpout != None and wpout not in products:
                products.append(wpout)
        elif script_id == 'modbus-discover':
            wpout = s.getAttribute('output')
            if wpout != None:
                ...
                products.append(prod)
        elif script_id == 'jenkins-version':
            wpout = s.getAttribute('output')
            if wpout and 'jenkins version number:' in wpout:
                ...
                if prodstr not in products:
                    products.append(prodstr)
        elif script_id == 'elasticsearch-version':
            wpout = s.getAttribute('output')
            ...
            if wpout and 'version:' in wpout:
                ...
                if prodstr not in products:
                    products.append(prodstr)
        elif script_id == 'grafana-version':
            wpout = s.getAttribute('output')
            if wpout and 'grafana version:' in wpout:
                ...
                if prodstr not in products:
                    products.append(prodstr)
        elif script_id == 'prometheus-version':
            wpout = s.getAttribute('output')
            ...
            if wpout and 'vault version:' in wpout:
                ...
                if prodstr not in products:
                    products.append(prodstr)
        elif script_id == 'influxdb-version':
            wpout = s.getAttribute('output')
            if wpout and 'influxdb version:' in wpout:
                ...
                if prodstr not in products:
                    products.append(prodstr)
        elif script_id == 'rabbitmq-version':
            wpout = s.getAttribute('output')
            if wpout and 'rabbitmq version:' in wpout:
                ...
                if prodstr not in products:
                    products.append(prodstr)
        elif script_id == 'sonarqube-version':
            wpout = s.getAttribute('output')
            ...
            if wpout and 'nexus version:' in wpout:
                ...
                if prodstr not in products:
                    products.append(prodstr)
        elif script_id == 'activemq-version':
            wpout = s.getAttribute('output')
            ...
            if wpout and 'etcd version:' in wpout:
                ...
                if prodstr not in products:
                    products.append(prodstr)
        elif script_id == 'couchdb-version':
            wpout = s.getAttribute('output')
        ...
        # --- web / HTTP detection ---
            ...
            if wpout and 'unknown' not in wpout.lower() and len(wpout.strip()) > 2:
                ...
                if prodstr not in products:
                    products.append(prodstr)
        elif script_id == 'http-qnap-nas-info':
            elems = s.getElementsByTagName('elem')
            model = ''
            firmware = ''
            for e in elems:
            ...
            if model:
                ...
                if ostype == 'Other':
                    ostype = 'QNAP'
        elif script_id == 'http-hp-ilo-info':
            elems = s.getElementsByTagName('elem')
            ilo_type = ''
            ilo_fw = ''
            server_type = ''
        ...
        elif script_id == 'http-trane-info':
            elems = s.getElementsByTagName('elem')
            product_name = ''
            product_ver = ''
            for e in elems:
            ...
            if product_name:
                ...
                if ostype == 'Other':
                    ostype = 'Trane'
        elif script_id == 'spring-boot-version':
            wpout = s.getAttribute('output')
            if wpout:
            ...
            if wpout:
                ...
                if prodstr not in products:
                    products.append(prodstr)
        elif script_id == 'keycloak-version':
            wpout = s.getAttribute('output')
            if wpout:
                ...
                if prodstr not in products:
                    products.append(prodstr)
        elif script_id == 'traefik-version':
            wpout = s.getAttribute('output')
            if wpout and 'traefik version:' in wpout:
                ...
                if prodstr not in products:
                    products.append(prodstr)
        elif script_id == 'portainer-version':
            wpout = s.getAttribute('output')
            if wpout and 'portainer version:' in wpout:
                ...
                if prodstr not in products:
                    products.append(prodstr)
        elif script_id == 'jupyter-version':
            wpout = s.getAttribute('output')
            if wpout:
                ...
                if prodstr not in products:
                    products.append(prodstr)
        elif script_id == 'weblogic-version':
            wpout = s.getAttribute('output')
            if wpout:
        ...
        # --- previously-scanned-but-unhandled built-in scripts ---
        elif script_id == 'kubernetes-version':
            elems = s.getElementsByTagName('elem')
            ver = None
            for e in elems:
        ...
        elif script_id == 'cassandra-info':
            wpout = s.getAttribute('output')
            if wpout:
                prodstr = 'apache cassandra'
                if prodstr not in products:
                    products.append(prodstr)
        elif script_id == 'db2-das-info':
            elems = s.getElementsByTagName('elem')
            ver = None
            for e in elems:
            ...
            if prodstr not in products:
                products.append(prodstr)
        elif script_id == 'oracle-tns-version':
            wpout = s.getAttribute('output')
            if wpout:
                ...
                if prodstr not in products:
                    products.append(prodstr)
        elif script_id == 'postgresql-info':
            wpout = s.getAttribute('output')
        ...
        # --- new service detection ---
            ...
            if wpout and 'nats version:' in wpout:
                ...
                if prodstr not in products:
                    products.append(prodstr)
        elif script_id == 'alertmanager-version':
            wpout = s.getAttribute('output')
            ...
            if wpout and 'zipkin version:' in wpout:
                ...
                if prodstr not in products:
                    products.append(prodstr)
        elif script_id == 'mattermost-version':
            wpout = s.getAttribute('output')
            if wpout and 'mattermost version:' in wpout:
                ...
                if prodstr not in products:
                    products.append(prodstr)
        elif script_id == 'rocketchat-version':
            wpout = s.getAttribute('output')
            ...
            if wpout and 'kong version:' in wpout:
                ...
                if prodstr not in products:
                    products.append(prodstr)
        elif script_id == 'artifactory-version':
            wpout = s.getAttribute('output')
        ...
        elif script_id == 'clickhouse-version':
            wpout = s.getAttribute('output')
        ...
        elif script_id == 'teamcity-version':
            wpout = s.getAttribute('output')
        ...
        elif script_id == 'victoria-metrics-version':
            wpout = s.getAttribute('output')
            ...
            if wpout and 'bamboo version:' in wpout:
                ...
                if prodstr not in products:
                    products.append(prodstr)
        elif script_id == 'cockroachdb-version':
            wpout = s.getAttribute('output')
        ...
        elif script_id == 'nextcloud-version':
            wpout = s.getAttribute('output')
            if wpout and 'nextcloud version:' in wpout:
                ...
                if prodstr not in products:
                    products.append(prodstr)
        elif script_id == 'arangodb-version':
            wpout = s.getAttribute('output')
...
def _resolve_os_name_tag(h, ostype, smb_os_name, products):
    os_name_tag = None
    ...
    if best_os_class:
        if ostype == "Linux":
            ...
            if smb_os_name:
                ...
                os_name_tag = os_name
    return products, os_name_tag


def _clean_products(products, ostype, smb_os_name):
    clean_products = []
    for p in products:
...
def nmap_scan(args, host):
    ...
    dom = parseString(out)
    asset_data_list = []
    for h in dom.getElementsByTagName("host"):
        try:
            addr = h.getElementsByTagName("address")[0].getAttribute('addr')
            hostname = addr
            harr = h.getElementsByTagName("hostname")
            if harr != None and len(harr) > 0:
                hostname = h.getElementsByTagName("hostname")[0].getAttribute('name')
                if hostname == 'linux':
                    hostname = addr

            ports_in_use_dict = _parse_host_ports(h)
            ssh_port_is_open = True if ports_in_use_dict.get(22) is not None else False

            products = _parse_host_cpes(h)
            ostype = get_os_type(h, products)
            products, ostype = _process_services(args, h, addr, products, ostype)
            products, ostype = _process_scripts(h, hostname, products, ostype)

            smb_os_name = get_smb_os(h)
            products, os_name_tag = _resolve_os_name_tag(h, ostype, smb_os_name, products)
            products = _clean_products(products, ostype, smb_os_name)
