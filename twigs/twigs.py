    ...
    if not args.test:
        ...
        if resp is None:
            ...
            if resp.status_code != 200:
                logging.info("Creating new asset [%s] [%s] [%s]", asset_id, asset['type'], asset.get('attack_surface_label'))
...
def run_remediation_scan(asset_id_list, args):
    ...
    scan_payload['issue_types'] = ["SAST", "IaC", "GCP CIS", "AWS CIS", "OCI CIS", "Azure CIS", "DigitalOcean CIS", "Host Benchmark"]
...
def main(args=None):
    ...
    try:
        ...
        parser_ecr.add_argument('--ssh_private_key', help=argparse.SUPPRESS, required=False)
        ...
        parser_ecr.add_argument('--use_trufflehog', action='store_true', help=argparse.SUPPRESS)
        ...
        parser_acr.add_argument('--ssh_private_key', help=argparse.SUPPRESS, required=False)
        ...
        parser_acr.add_argument('--use_trufflehog', action='store_true', help=argparse.SUPPRESS)
        ...
        parser_gcr.add_argument('--ssh_private_key', help=argparse.SUPPRESS, required=False)
        ...
        parser_gcr.add_argument('--use_trufflehog', action='store_true', help=argparse.SUPPRESS)
        ...
        parser_ocr.add_argument('--ssh_private_key', help=argparse.SUPPRESS, required=False)
        ...
        parser_ocr.add_argument('--use_trufflehog', action='store_true', help=argparse.SUPPRESS)
        ...
        parser_docker.add_argument('--ssh_private_key', help=argparse.SUPPRESS, required=False)
        ...
        parser_docker.add_argument('--use_trufflehog', action='store_true', help=argparse.SUPPRESS)
        ...
        parser_k8s.add_argument('--ssh_private_key', help=argparse.SUPPRESS, required=False)
        ...
        parser_k8s.add_argument('--use_trufflehog', action='store_true', help=argparse.SUPPRESS)
        ...
        parser_repo.add_argument('--ssh_private_key', help='Private key file path for git authentication over ssh', required=False)
        ...
        parser_repo.add_argument('--use_trufflehog', action='store_true', help=argparse.SUPPRESS)
        ...
        parser_ghe.add_argument('--ssh_private_key', help=argparse.SUPPRESS, required=False)
        ...
        parser_ghe.add_argument('--use_trufflehog', action='store_true', help=argparse.SUPPRESS)
        ...
        parser_gle.add_argument('--ssh_private_key', help=argparse.SUPPRESS, required=False)
        ...
        parser_gle.add_argument('--use_trufflehog', action='store_true', help=argparse.SUPPRESS)
        ...
        parser_bb.add_argument('--ssh_private_key', help=argparse.SUPPRESS, required=False)
        ...
        parser_bb.add_argument('--use_trufflehog', action='store_true', help=argparse.SUPPRESS)
        ...
        parser_az_functions.add_argument('--use_trufflehog', action='store_true', help=argparse.SUPPRESS)
        ...
        parser_gcloud_functions.add_argument('--ssh_private_key', help=argparse.SUPPRESS, required=False)
        ...
        parser_gcloud_functions.add_argument('--use_trufflehog', action='store_true', help=argparse.SUPPRESS)
        ...
        parser_easm.add_argument('--fqdn', help='Fully qualified hostname or domain name to assess (shorthand for a single domain/hostname --seed). At least one of --fqdn / --seed / --seed_file is required')
        parser_easm.add_argument('--seed', action='append', metavar='SEED', help='Discovery seed: a domain, hostname, IP address, CIDR block, or ASN (e.g. AS14618 or 14618). Repeatable. Domain-level checks (WHOIS, email, DNS hygiene, typosquatting, subdomain enumeration) run only for seeded domains; IP/CIDR/ASN seeds get host-level assessment only')
        parser_easm.add_argument('--seed_file', help='Path to a file of seeds, one per line (domain/hostname/IP/CIDR/ASN); blank lines and lines starting with # are ignored')
        parser_easm.add_argument('--max_seed_hosts', help=argparse.SUPPRESS, default=256, type=int)
        parser_easm.add_argument('--assetname', help='Optional name/label to be assigned to the primary discovered asset')
        parser_easm.add_argument('--quick', action='store_true', help='Quick domain scan: a fast, passive / light-touch subset of the EASM checks for a single domain (passive subdomain discovery, email/DNS hygiene, takeover, TLS certificate, security headers, tech stack, WAF, WHOIS, lookalike domains, exposed risky services). Includes a named-vulnerability TLS audit (time-boxed per host). Skips nmap service probing, nuclei, content discovery, DNS brute force and netblock sweeps')
        parser_easm.add_argument('--quick_timeout', help=argparse.SUPPRESS, default=300, type=int)
        parser_easm.add_argument('--no_subdomain_bruteforce', action='store_true', help=argparse.SUPPRESS)
        ...
        parser_easm.add_argument('--no_cloud_attribution', action='store_true', help=argparse.SUPPRESS)
        parser_easm.add_argument('--cloud_ranges_ttl', help=argparse.SUPPRESS, default=86400, type=int)
        parser_easm.add_argument('--no_kev_epss', action='store_true', help=argparse.SUPPRESS)
        parser_easm.add_argument('--kev_epss_ttl', help=argparse.SUPPRESS, default=86400, type=int)
        ...
        parser_easm.add_argument('--typosquat_dns_limit', help=argparse.SUPPRESS, default=1000, type=int)
        ...
        parser_easm.add_argument('--wordlist_tier', choices=['small', 'medium', 'large'], default='small', help="DNS brute-force wordlist size: 'small' (~130 curated labels, default), 'medium' (~5k), 'large' (~20k). Larger tiers find more subdomains but take longer.")
        parser_easm.add_argument('--wordlist_file', help='Path to a custom newline-separated subdomain label wordlist (overrides --wordlist_tier)')
        parser_easm.add_argument('--dns_workers', help=argparse.SUPPRESS, default=20, type=int)
        parser_easm.add_argument('--no_ct_monitor', action='store_true', help=argparse.SUPPRESS)
        parser_easm.add_argument('--ct_monitor_window', help=argparse.SUPPRESS, default=93600, type=int)
        parser_easm.add_argument('--no_subdomain_permutations', action='store_true', help=argparse.SUPPRESS)
        parser_easm.add_argument('--permutation_limit', help=argparse.SUPPRESS, default=3000, type=int)
        parser_easm.add_argument('--no_version_probe', action='store_true', help=argparse.SUPPRESS)
        parser_easm.add_argument('--no_content_discovery', action='store_true', help=argparse.SUPPRESS)
        parser_easm.add_argument('--content_discovery_limit', help=argparse.SUPPRESS, default=15000, type=int)
        parser_easm.add_argument('--no_reverse_ip', action='store_true', help=argparse.SUPPRESS)
        parser_easm.add_argument('--reverse_ip_all', action='store_true', help=argparse.SUPPRESS)
        parser_easm.add_argument('--no_reverse_whois', action='store_true', help=argparse.SUPPRESS)
        parser_easm.add_argument('--reverse_whois_org', help=argparse.SUPPRESS)
        parser_easm.add_argument('--no_resolver_pool', action='store_true', help=argparse.SUPPRESS)
        parser_easm.add_argument('--resolver_pool', help=argparse.SUPPRESS)
        parser_easm.add_argument('--no_js_analysis', action='store_true', help=argparse.SUPPRESS)
        parser_easm.add_argument('--no_bucket_discovery', action='store_true', help=argparse.SUPPRESS)
        parser_easm.add_argument('--no_saas_discovery', action='store_true', help=argparse.SUPPRESS)
        parser_easm.add_argument('--saas_slug', action='append', help='Explicit SaaS tenant slug(s) to probe for (repeatable); overrides the auto-derived organisation label')
        parser_easm.add_argument('--saas_discovery_ttl', help=argparse.SUPPRESS, default=604800, type=int)
        parser_easm.add_argument('--github_repo_scan', action='store_true', help="During SaaS tenant discovery, download the discovered GitHub org's public repositories and scan them for leaked secrets (opt-in; uses GITHUB_TOKEN / --github_token when available)")
        parser_easm.add_argument('--github_token', help=argparse.SUPPRESS)
        parser_easm.add_argument('--github_max_repos', help=argparse.SUPPRESS, default=15, type=int)
        parser_easm.add_argument('--github_max_repo_mb', help=argparse.SUPPRESS, default=20, type=int)
        parser_easm.add_argument('--no_netblock_sweep', action='store_true', help=argparse.SUPPRESS)
        parser_easm.add_argument('--no_portsweep', action='store_true', help=argparse.SUPPRESS)
        parser_easm.add_argument('--portsweep_ports', help=argparse.SUPPRESS)
        parser_easm.add_argument('--portsweep_rate', help=argparse.SUPPRESS, default=2000, type=int)
        parser_easm.add_argument('--asn_sweep', action='store_true', help='Derive the target organisation\'s own ASNs from its IPs and reverse-DNS / port sweep their announced prefixes (broader discovery, slower)')
        parser_easm.add_argument('--recursive_discovery', action='store_true', help='After the main pass, run one or more additional discovery rounds on high-confidence related domains (reverse-WHOIS siblings, co-hosted domains on owned IPs, internal hostnames from JS)')
        parser_easm.add_argument('--max_discovery_depth', help=argparse.SUPPRESS, default=1, type=int)
        parser_easm.add_argument('--max_discovered_domains', help=argparse.SUPPRESS, default=10, type=int)
        ...
        parser_easm.add_argument('--otx_api_key', help=argparse.SUPPRESS)
        parser_easm.add_argument('--urlscan_api_key', help=argparse.SUPPRESS)
        parser_easm.add_argument('--no_zone_walk', action='store_true', help=argparse.SUPPRESS)
        parser_easm.add_argument('--no_graphql_enum', action='store_true', help=argparse.SUPPRESS)
        parser_easm.add_argument('--no_openapi_probe', action='store_true', help=argparse.SUPPRESS)
        parser_easm.add_argument('--no_dir_brute', action='store_true', help=argparse.SUPPRESS)
        parser_easm.add_argument('--dir_brute_wordlist', help='Path to a custom newline-separated path list for the calibrated content brute force (overrides the built-in curated list)')
        parser_easm.add_argument('--no_tls_fingerprint', action='store_true', help=argparse.SUPPRESS)
        parser_easm.add_argument('--no_udp_scan', action='store_true', help=argparse.SUPPRESS)
        parser_easm.add_argument('--udp_scan_ports', help=argparse.SUPPRESS)
        parser_easm.add_argument('--no_ipv6', action='store_true', help=argparse.SUPPRESS)
        parser_easm.add_argument('--ipv6_scan_all_hosts', action='store_true', help="Run a full nmap -6 service scan of every discovered host's IPv6 address (default: primary host only; other hosts get a fast IPv6 TCP port sweep)")
...
#        sys.exit(1)
        ...
        if args.mode == 'aws':
            ...
            if not (args.fqdn or getattr(args, 'seed', None) or getattr(args, 'seed_file', None)):
                logging.error("easm mode requires at least one of --fqdn, --seed or --seed_file")
                sys.exit(1)
