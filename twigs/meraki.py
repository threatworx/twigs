...
def get_organizations(args, headers):
   url = args.base_url + '/organizations'
   resp = requests.get(url, headers=headers)
   resp.raise_for_status()
   return resp.json()

def get_networks(args, headers, org_id):
   url = args.base_url + '/organizations/{org_id}/networks'
   resp = requests.get(url, headers=headers, verify=False)
   resp.raise_for_status()
   return resp.json()

def get_devices(args, headers, org_id):
   url = args.base_url + '/organizations/{org_id}/devices'
   resp = requests.get(url, headers=headers, verify=False)
   resp.raise_for_status()
   return resp.json()

def get_firmware_upgrades(args, headers, network_id):
   url = args.base_url + '/networks/{network_id}/firmwareUpgrades'
   resp = requests.get(url, headers=headers, verify=False)
...
def get_all_devices(args, headers):
    ...
    for org in organizations:
        logging.info("Organization: {org['name']} ({org['id']})")
        ...
        for net in networks:
            logging.info("Network: {net['name']} ({net['id']})")
            ...
            for device in devices_by_network.get(net['id'], []):
                ...
                logging.info("Device: {device.get('name', device['serial'])} | Model: {device['model']} | Product: {product_type} | Firmware: {fw_version}")
                asset = {}
